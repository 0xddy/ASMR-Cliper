...
声音勾选框改用独立的 `DrawCheckboxControl`：GDI+ 浮点路径绘制 20 DIP 圆角方框、1.15 DIP 描边与 1.9 DIP 圆端勾号，按 DPI 缩放；沿用整个子控件缓冲提交，文字和背景保持静止。未选描边提高辨识度，悬停只调整方框颜色，按下略微收缩，禁用使用独立浅色状态。焦点不会添加整行矩形背景或虚线框。

勾选与取消共用 180 ms 缓出动画，底色渐变、勾号沿短边再长边逐步显示；连续点击从当前进度反向。逻辑值和自动保存即时生效，不等待动画结束。遵循系统动画设置，初始化、重置、隐藏、禁用和销毁会结束动画；鼠标离开清除悬停状态。复用原生按钮的鼠标与空格操作。

GUI 回归覆盖实际勾选保存、空格、动画中间状态及反向、隐藏 / 禁用 / 重置、悬停恢复，以及 100% / 125% / 150% / 200% / 250% 的边缘像素和干净背景。音频分析、保留默认值与导出逻辑不变。

## 0.6.17 轻语 / 耳语保留选项

`keep_whisper` 默认 true，设置页与轻笑同行，八项声音保持两列四行；修改即时保存、关闭后恢复去除耳语，重置恢复勾选。引擎与 GUI 均把启用此选项所需的 CLAP / neural 计入环境要求。

成片节目单增加“轻语 / 耳语”独立声音描述和名称，普通说话仍单列；节目单继续根据成片声音识别，不用设置顺序合成或重排音频。

`whispering.detect` 在初步语音 / 声学排除后、饮水与 V4 正向提取之前检查完整的非音乐区域，避免先被 ASR 标记的话语永远没有保留机会。AST 单列 Whispering 与普通语音分数，缓存升级 events-v6。10 秒 / 5 秒步长寻找声学候选，3 秒 / 1 秒步长同时比较耳语、普通说话与其他 ASMR 描述；需要多个连续窗口支持，只赋予窗口中心保留许可，不跨过缺少证据的窗口。低音量、转写文本或单次命中不会自动获得许可。

许可从初次话语、声学人声和严格模式的完整句子区间中扣除，并在审计和成片复核后重新规划时一致应用。分类缓存保留原始分数，仅在查询结果完全位于许可内时附加上下文标记；明确的普通语音或情绪声音仍阻止边界放行。V4 将已确认轻语作为独立正向证据，同时继续排除音乐、饮水和其他不保留的声音。

成片 ASR 缓存始终保存原始话语发现，`exporter` 每次按当前开关和最终音视频的 analysis/output 映射应用许可。只豁免已确认轻语范围，跨界语句拆开保留其他疑似位置；不会因开关变化复用旧的“通过”判定，也不掩盖模型错误。该步骤沿用源音轨已有的声音样式证据，没有额外对成片做一次样式模型推理。结果保存原片 `whisper_review`、成片 `whisper_retained`、`allowed_whisper` 与 `轻语保留.csv`；中间 `whisper-review.json` 纳入任务完成清理。

回归覆盖默认值、持久化、类别重置、独立人声分数与缓存、连续证据及混合边界、三种模式和重规划、V4 排除优先级、音视频实际时间映射、关闭开关后的同一复核结果，以及实际 AAC 导出。模型根据声音样式判别，不能保证识别表演意图；轻声聊天与 ASMR 耳语仍可能混淆。
