...
def export(source, output_dir, meta, frames, plan, cfg, source_fingerprint, reviewer=None, allow_review_findings=False,
    ...
    try:
        ...
        with (staging/'轻语保留.csv').open('w',encoding='utf-8-sig',newline='') as f:
            writer=csv.writer(f);writer.writerow(['成片开始','成片结束','保留依据'])
            for a,b in report['whisper_retained']:writer.writerow([clock(a),clock(b),'轻语 / 耳语声音证据，且已勾选保留'])
...
普通说话继续删除；默认勾选“轻语 / 耳语”，只保留有相应声音证据的区间，成片复核遵循同一选择。默认还保留轻笑、心跳和 ASMR 道具敲击。轻语保留.csv 使用成片时间，原片证据见 whisper_review；保留选项见校验报告 settings。
