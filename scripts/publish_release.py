...
def validate_archive(archive, checksum, commit):
    match = re.fullmatch(r'ASMR-Cliper-(\d+\.\d+\.\d+)-win64-nv\.zip', archive.name)
    if not match:
...
def publish(archive, checksum, repository, commit, run_id, part_size=PART_SIZE):
    ...
    version, archive_hash = validate_archive(archive, checksum, commit)
    count = (archive.stat().st_size + part_size - 1) // part_size
    if count > 990:
        raise ValueError('Too many Release assets.')
    if shutil.disk_usage(archive.parent).free < min(part_size, archive.stat().st_size) + 128 * 1024**2:
        raise RuntimeError('Release staging needs space for one part plus 128 MiB.')
    tag = f'v{version}-win64-nv-{run_id}'
    ...
    with tempfile.TemporaryDirectory(prefix='release-assets-', dir=archive.parent) as temp:
        ...
        if release is None:
            gh('release', 'create', tag, '--repo', repository, '--target', commit, '--draft',
               '--title', f'ASMR-Cliper {version} · win64-nv', '--notes-file', notes)
            release = read_release(repository, tag)
        ...
        for path in (manifest_path, merge, checksum_copy):
...
def main():
    ...
    url = publish(args.archive, args.checksum, args.repository, args.commit, args.run_id)
