...
std::vector<std::string> MainWindow::requiredComponents() const {
    ...
    const bool reviewing=cfg_.value("review_enabled",true)||cfg_.value("mode","")=="extract";
    if(reviewing)add(cfg_.value("review_model_id","whisper-large-v3"));
    const bool qwenReview=reviewing&&cfg_.value("review_model_id","whisper-large-v3")=="qwen3-asr";
    if(cfg_.value("mode","")=="extract"||!cfg_.value("keep_drinking",false)||cfg_.value("keep_whisper",true)||qwenReview){result.push_back("clap");result.push_back("neural");}
    return result;
}
