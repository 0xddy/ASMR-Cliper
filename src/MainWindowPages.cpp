...
void MainWindow::paint(HDC dc) {
    ...
    label(L"ASMR-Cliper",54,28,142,32,brandFont_);label(L"v0.6.9",24,h-43,140,20,smallFont_,Muted);
    ...
    if(page_==0) {
        ...
        if(settingsTab_==0) {
            ...
            const wchar_t* notes[]={L"关闭两项淡化可复制原音频包",L"视频使用 MKV 容器",L"未压缩音频 · 视频使用 MKV 容器",L"无损输入默认每声道 128 kbps"};
